...
public class RbacInitializer implements CommandLineRunner {
     ...
     * false：只补齐权限定义（sys_permission），不改角色授权，避免覆盖库内永久化配置。
     */
    private boolean seedRolePermissions;

    @Override
    @Transactional
    public void run(String... args) {
        permissionRepository.migrateLegacySearchAndTableTypes();
        seedRolePermissions = permissionRepository.count() == 0;
        ...
        ensureRemoteStoragePermissions();
    ...
    private void ensureSystemConfigPermissions() {
        Permission settings =
                permissionRepository
                        ...
                        .orElse(null);
        Permission menu =
                ensureMenuPermission("menu:system:config", "系统配置", "/system/config", settings, 2);
        ...
        upsertApi(
                ...
                menu,
                2);
        // 每个分区一套独立的读写接口（storage 已拆到「远程连接配置」）
        int sort = 3;
        for (String[] section :
                new String[][] {
                    {"app", "应用信息"},
            ...
            String path = "/api/system-config/" + section[0];
            upsertApi("api:GET:" + path, "系统配置-" + section[1] + "查询", "GET", path, menu, sort++);
            upsertApi("api:PUT:" + path, "系统配置-" + section[1] + "保存", "PUT", path, menu, sort++);
        }
        removeObsoletePermission("api:PUT:/api/system-config");
        removeObsoletePermission("api:GET:/api/system-config/{section}");
        removeObsoletePermission("api:PUT:/api/system-config/{section}");
        // 旧对象存储按钮挂在系统配置下，拆页后清理
        removeObsoletePermission("system-config:storage-create");
        removeObsoletePermission("system-config:storage-update");
        removeObsoletePermission("system-config:storage-view");
        removeObsoletePermission("system-config:storage-delete");
        removeObsoletePermission("system-config:table-storage-edit");
        removeObsoletePermission("system-config:table-storage-view");
        removeObsoletePermission("system-config:table-storage-delete");
        // /public 走 permitAll，不登记到角色
    }

    /** 远程连接配置：名称 / 路径映射，覆盖前端 appConfig.storage */
    private void ensureRemoteStoragePermissions() {
        Permission settings =
                permissionRepository
                        .findByCode("menu:system:settings")
                        .or(() -> permissionRepository.findByCode("menu:system"))
        ...
        Permission menu =
                ensureMenuPermission(
                        "menu:system:remote-storage",
                        "远程连接配置",
                        "/system/remote-storage",
                        settings,
                        3);
        ensureDualCrudButtons(menu, "remote-storage", false, false);
        upsertApi(
                "api:GET:/api/system-config/storage",
                "远程连接配置查询",
                "GET",
                "/api/system-config/storage",
                menu,
                1);
        upsertApi(
                ...
                menu,
                2);
        inheritRemoteStorageGrants();
    }

    /**
     * 远程连接配置由「系统配置」的对象存储页签拆分而来，已有库不会自动获得新权限，导致侧栏看不到该菜单。
     * 这里把新权限补授给原本已能访问系统配置的角色；一旦有任意角色持有该菜单权限即视为已迁移，后续启动不再改动授权。
     */
    private void inheritRemoteStorageGrants() {
        ...
        boolean alreadyMigrated =
                roles.stream()
                        ...
                        .anyMatch(p -> "menu:system:remote-storage".equals(p.getCode()));
        if (alreadyMigrated) {
            return;
        }
        for (Role role : roles) {
            boolean ownsSystemConfig =
                    role.getPermissions().stream()
                            .anyMatch(p -> "menu:system:config".equals(p.getCode()));
            if (!ownsSystemConfig) {
                continue;
            }
            Set<Permission> perms = new HashSet<>(role.getPermissions());
            if (perms.addAll(granted)) {
    ...
    private void ensureSiteContactPermissions() {
        ...
        Permission menu =
                ensureMenuPermission(
                        "menu:system:site-contact", "联系与捐赠", "/system/site-contact", settings, 5);
    ...
    private void ensureSecurityPolicyPermissions() {
        ...
        Permission menu =
                ensureMenuPermission(
                        "menu:system:security", "安全策略", "/system/security", settings, 4);
        ...
        upsertApi(
                "api:GET:/api/security-policy/locks",
                "锁定账号列表",
                "GET",
                "/api/security-policy/locks",
                menu,
                3);
        upsertApi(
                ...
                "解锁账号",
                "DELETE",
                "/api/security-policy/locks/{username}",
                menu,
                4);
    }
    ...
    private void ensureDictPermissions() {
        Permission base =
                permissionRepository
                        .findByCode("menu:system:base")
                        .or(() -> permissionRepository.findByCode("menu:system"))
                        .orElse(null);
        Permission menu =
                ensureMenuPermission("menu:system:dict", "字典管理", "/system/dicts", base, 1);
        ensureDualCrudButtons(menu, "dict-type", false, false);
        upsertTableButton("dict-type:table-data", "字典数据", menu, 4);
        permissionRepository
                .findByCode("dict-type:table-data")
        ...
        upsertApi(
                ...
                "字典类型选项",
                "GET",
                "/api/dict-types/options",
                menu,
                2);
        upsertApi(
                "api:GET:/api/dict-types/{id}", "字典类型详情接口", "GET", "/api/dict-types/{id}", menu, 3);
        upsertApi("api:POST:/api/dict-types", "创建字典类型接口", "POST", "/api/dict-types", menu, 4);
        upsertApi(
                "api:PUT:/api/dict-types/{id}", "更新字典类型接口", "PUT", "/api/dict-types/{id}", menu, 5);
        upsertApi(
                "api:DELETE:/api/dict-types/{id}",
                "删除字典类型接口",
                "DELETE",
                "/api/dict-types/{id}",
                menu,
                6);
        upsertApi(
                "api:POST:/api/dict-types/batch-delete",
                "批量删除字典类型",
                "POST",
                "/api/dict-types/batch-delete",
                menu,
                7);

        Permission dataMenu =
                ensureMenuPermission(
                        "menu:system:dict-data", "字典数据", "/system/dicts/data", menu, 1);
        ...
        upsertApi(
                ...
                "按类型取字典数据",
                "GET",
                "/api/dict-data/type/{dictType}",
                dataMenu,
                2);
        upsertApi(
                "api:GET:/api/dict-data/{id}",
                "字典数据详情接口",
                "GET",
                "/api/dict-data/{id}",
                dataMenu,
        ...
        upsertApi(
                ...
                6);
        upsertApi(
                "api:POST:/api/dict-data/batch-delete",
                "批量删除字典数据",
                "POST",
                "/api/dict-data/batch-delete",
                dataMenu,
                7);
    ...
    private void ensureLoginPagePermissions() {
        ...
        upsertApi(
                "api:GET:/api/login-page-configs",
                "登录页配置列表",
                "GET",
                "/api/login-page-configs",
                menu,
                1);
        upsertApi(
                "api:GET:/api/login-page-configs/{id}",
                "登录页配置详情",
                "GET",
                "/api/login-page-configs/{id}",
                menu,
                2);
        upsertApi(
                "api:POST:/api/login-page-configs",
                "创建登录页配置",
                "POST",
                "/api/login-page-configs",
                menu,
                3);
        upsertApi(
                "api:PUT:/api/login-page-configs/{id}",
                "更新登录页配置",
                "PUT",
                "/api/login-page-configs/{id}",
                menu,
                4);
        upsertApi(
                "api:PUT:/api/login-page-configs/{id}/status",
                "启停登录页配置",
                "PUT",
                "/api/login-page-configs/{id}/status",
                menu,
                5);
        upsertApi(
                "api:DELETE:/api/login-page-configs/{id}",
                "删除登录页配置",
                "DELETE",
                "/api/login-page-configs/{id}",
                menu,
                6);
        upsertApi(
                "api:POST:/api/login-page-configs/batch-delete",
                "批量删除登录页配置",
                "POST",
    ...
    private void ensureLoginLogPermissions() {
        ...
        Permission menu =
                ensureMenuPermission(
                        "menu:system:login-log", "登录日志", "/system/logs/login", logs, 1);
        ...
        upsertApi(
                ...
                2);
        upsertApi(
                "api:POST:/api/logs/login/batch-delete",
                "批量删除登录日志",
                "POST",
                "/api/logs/login/batch-delete",
                menu,
                3);
        upsertApi(
                "api:DELETE:/api/logs/login/clean",
                "清空登录日志接口",
                "DELETE",
                "/api/logs/login/clean",
                menu,
                4);
        upsertApi(
                "api:GET:/api/logs/login/export",
                "导出登录日志",
                "GET",
                "/api/logs/login/export",
                menu,
                5);
    }
    ...
    private void ensureOperLogPermissions() {
        Permission logs = ensureLogsGroup();
        Permission menu =
                ensureMenuPermission("menu:system:oper-log", "操作日志", "/system/logs/oper", logs, 2);
        ...
        upsertApi(
                ...
                3);
        upsertApi(
                "api:POST:/api/logs/oper/batch-delete",
                "批量删除操作日志",
                "POST",
                "/api/logs/oper/batch-delete",
                menu,
                4);
        upsertApi(
                "api:DELETE:/api/logs/oper/clean",
                "清空操作日志接口",
                "DELETE",
                "/api/logs/oper/clean",
                menu,
                5);
        upsertApi(
                "api:GET:/api/logs/oper/export", "导出操作日志", "GET", "/api/logs/oper/export", menu, 6);
    }

    /** 授权指定接口权限给全部启用角色（如全站通用的只读接口）。仅空库首次种子执行。 */
    ...
    private void ensureMonitorPermissions() {
        ...
        Permission online =
                ensureMenuPermission("menu:monitor:online", "在线用户", "/monitor/online", monitor, 1);
        Permission server =
                ensureMenuPermission("menu:monitor:server", "服务监控", "/monitor/server", monitor, 2);
        ...
        upsertApi(
                "api:POST:/api/monitor/online/{userId}/kick",
                "强制下线",
                "POST",
                "/api/monitor/online/{userId}/kick",
                online,
                2);
        ...
        upsertApi(
                "api:POST:/api/monitor/infra/{name}/restart",
                "基础设施一键重启",
                "POST",
                "/api/monitor/infra/{name}/restart",
                server,
                3);
    }
    ...
    private void ensureRedisSqlMonitorPermissions() {
        Permission monitor = permissionRepository.findByCode("menu:monitor").orElse(null);
        Permission redis =
                ensureMenuPermission("menu:monitor:redis", "缓存监控", "/monitor/redis", monitor, 3);
        Permission sql =
        ...
        upsertApi(
                "api:DELETE:/api/monitor/redis/keys",
                "删除Redis键",
                "DELETE",
                "/api/monitor/redis/keys",
                redis,
                2);
        upsertApi(
                "api:DELETE:/api/monitor/redis/flush",
                "清空Redis库",
                "DELETE",
                "/api/monitor/redis/flush",
                redis,
                3);

        upsertApi("api:GET:/api/monitor/sql", "SQL监控数据", "GET", "/api/monitor/sql", sql, 1);
        upsertApi(
                "api:DELETE:/api/monitor/sql/clean",
                "清空SQL监控",
                "DELETE",
                "/api/monitor/sql/clean",
                sql,
                2);
        upsertApi(
                "api:DELETE:/api/monitor/sql/records/{id}",
                "删除SQL记录",
                "DELETE",
                "/api/monitor/sql/records/{id}",
                sql,
                3);
    }

    private void ensureMessagePermissions() {
        Permission menuContent =
                permissionRepository.findByCode("menu:system:content").orElse(null);
        ...
        Permission menu =
                ensureMenuPermission("menu:system:message", "站内信", "/system/messages", parent, 2);
        ...
        upsertApi(
                "api:DELETE:/api/messages/{id}", "删除站内信", "DELETE", "/api/messages/{id}", menu, 5);
        upsertApi(
                "api:POST:/api/messages/batch-delete",
                "批量删除站内信",
                "POST",
        ...
        upsertApi(
                ...
                7);
        upsertApi(
                "api:GET:/api/messages/{id}/readers",
                "站内信已读明细",
                "GET",
                "/api/messages/{id}/readers",
                menu,
                8);
        ...
        Permission mineMenu =
                ensureMenuPermission(
                        "menu:personal:message", "我的消息", "/messages/mine", personal, 2);
        ...
        upsertApi(
                ...
                "批量删除我的消息",
                "POST",
                "/api/messages/mine/batch-delete",
                mineMenu,
                5);
    ...
    private void ensureExceptionLogPermissions() {
        ...
        Permission menu =
                ensureMenuPermission(
                        "menu:system:exception-log", "异常日志", "/system/logs/exception", logs, 3);
        ...
        upsertApi(
                ...
                2);
        upsertApi(
                "api:DELETE:/api/logs/exception/{id}",
                "删除异常日志",
                "DELETE",
                "/api/logs/exception/{id}",
                menu,
                3);
        upsertApi(
                "api:POST:/api/logs/exception/batch-delete",
                "批量删除异常日志",
                "POST",
                "/api/logs/exception/batch-delete",
                menu,
                4);
        upsertApi(
                "api:DELETE:/api/logs/exception/clean",
                "清空异常日志",
                "DELETE",
                "/api/logs/exception/clean",
                menu,
                5);
        upsertApi(
                "api:GET:/api/logs/exception/export",
                "导出异常日志",
                "GET",
                "/api/logs/exception/export",
                menu,
                6);
    }
    ...
    private void ensureFilePermissions() {
        Permission tools =
                permissionRepository
                        ...
                        .orElse(null);
        Permission menu =
                ensureMenuPermission("menu:system:file", "文件管理", "/system/files", tools, 1);
        ...
        // 大文件分片上传：与 file:upload 共用按钮权限，接口需单独登记否则被接口守卫拦截
        upsertApi(
                "api:POST:/api/files/chunk/check",
                "分片上传秒传探测",
                "POST",
                "/api/files/chunk/check",
                menu,
                7);
        upsertApi(
                "api:POST:/api/files/chunk/init",
                "初始化分片上传",
                "POST",
                ...
                8);
        upsertApi(
                "api:GET:/api/files/chunk/{uploadId}/status",
                "分片上传状态",
                "GET",
                "/api/files/chunk/{uploadId}/status",
                menu,
                9);
        upsertApi(
                "api:POST:/api/files/chunk/{uploadId}/part",
                "上传分片",
                "POST",
                "/api/files/chunk/{uploadId}/part",
                menu,
                10);
        upsertApi(
                "api:POST:/api/files/chunk/{uploadId}/complete",
                "合并分片",
                "POST",
                "/api/files/chunk/{uploadId}/complete",
                menu,
                11);
        upsertApi(
                "api:DELETE:/api/files/chunk/{uploadId}",
                "取消分片上传",
                "DELETE",
                "/api/files/chunk/{uploadId}",
                menu,
                12);
    }
    ...
    private void ensureRecyclePermissions() {
        Permission tools =
                permissionRepository
                        ...
                        .orElse(null);
        Permission menu =
                ensureMenuPermission("menu:system:recycle", "回收站", "/system/recycle", tools, 5);
        ...
        upsertApi(
                ...
                2);
        upsertApi(
                "api:DELETE:/api/recycle/{id}", "彻底删除回收站项", "DELETE", "/api/recycle/{id}", menu, 3);
        upsertApi(
                "api:POST:/api/recycle/batch-delete",
                "批量彻底删除",
                "POST",
                "/api/recycle/batch-delete",
                menu,
                4);
        upsertApi(
                "api:DELETE:/api/recycle/clean", "清空回收站", "DELETE", "/api/recycle/clean", menu, 5);
    }
    ...
    private void ensureJobPermissions() {
        ...
        upsertApi(
                "api:POST:/api/jobs/batch-delete",
                "批量删除定时任务",
                "POST",
                ...
                6);
        upsertApi(
                "api:PUT:/api/jobs/{id}/status", "启停定时任务", "PUT", "/api/jobs/{id}/status", menu, 7);
    ...
    private void ensureCodegenPermissions() {
        Permission tools =
                permissionRepository
                        ...
                        .orElse(null);
        Permission menu =
                ensureMenuPermission("menu:system:codegen", "代码生成", "/system/codegen", tools, 6);
        ...
        upsertApi(
                "api:GET:/api/codegen/tables/{tableName}/columns",
                "代码生成-表字段",
                "GET",
                "/api/codegen/tables/{tableName}/columns",
                menu,
                2);
        upsertApi(
                "api:POST:/api/codegen/preview",
                "代码生成-预览",
                "POST",
                "/api/codegen/preview",
                menu,
    ...
    private Permission ensureMenuPermission(
            String code, String name, String path, Permission parent, int sort) {
    ...
    private void ensurePostPermissions() {
        ...
        Permission menu =
                ensureMenuPermission("menu:system:post", "岗位管理", "/system/posts", parent, 3);
    ...
    private void ensureJobLogPermissions() {
        Permission tools =
                permissionRepository
                        ...
                        .orElse(null);
        Permission menu =
                ensureMenuPermission("menu:system:job-log", "任务日志", "/system/jobs/logs", tools, 3);
        ...
        upsertApi(
                "api:DELETE:/api/logs/job/{id}", "删除任务日志", "DELETE", "/api/logs/job/{id}", menu, 4);
        upsertApi(
                "api:POST:/api/logs/job/batch-delete",
                "批量删除任务日志",
                "POST",
                "/api/logs/job/batch-delete",
                menu,
                5);
        upsertApi(
                "api:DELETE:/api/logs/job/clean",
                "清空任务日志",
                "DELETE",
                "/api/logs/job/clean",
                menu,
    ...
    private void ensureUnitPermissions() {
        ...
        if (menu == null) {
            int nextSort =
                    (menuUser != null && menuUser.getSort() != null ? menuUser.getSort() : 0) + 1;
            Permission created = new Permission();
            ...
            menu = permissionRepository.save(created);
                ...
                && (menu.getParent() == null || !menu.getParent().getId().equals(parent.getId()))) {
            menu.setParent(parent);
            permissionRepository.save(menu);
        }
        grantToPrivilegedRoles(menu);
        ...
        upsertApi(
                ...
                8);
        upsertApi(
                "api:PUT:/api/units/{id}/roles", "分配单位角色", "PUT", "/api/units/{id}/roles", menu, 9);
    }
    ...
    private void ensureNoticePermissions() {
        ...
        for (String code :
                List.of("notice:table-publish", "notice:table-revoke", "notice:table-readers")) {
            permissionRepository.findByCode(code).ifPresent(this::grantToPrivilegedRoles);
        }
        ...
        upsertApi(
                "api:POST:/api/notices/{id}/publish",
                "下发公告接口",
                "POST",
                ...
                6);
        upsertApi(
                "api:POST:/api/notices/{id}/revoke",
                "撤回公告接口",
                "POST",
                "/api/notices/{id}/revoke",
                menu,
                7);
        upsertApi(
                "api:GET:/api/notices/{id}/readers",
                "公告已读明细",
                "GET",
                "/api/notices/{id}/readers",
                menu,
                8);
        upsertApi(
                "api:POST:/api/notices/batch-delete",
                "批量删除公告",
                "POST",
                "/api/notices/batch-delete",
                menu,
                9);
        upsertApi(
                "api:POST:/api/notices/batch-publish",
                "批量下发公告",
                "POST",
                "/api/notices/batch-publish",
                menu,
                10);
        upsertApi(
                "api:POST:/api/notices/batch-revoke",
                "批量撤回公告",
                "POST",
                "/api/notices/batch-revoke",
                menu,
                11);
        upsertApi(
    ...
    /** 我的公告 / 已读回执：全部启用角色可读。仅空库首次种子执行。 */
    private void grantNoticeInboxToAllRoles() {
        if (!seedRolePermissions) {
            return;
        }
        for (String code :
                List.of("api:GET:/api/notices/mine", "api:POST:/api/notices/{id}/read")) {
            Permission permission = permissionRepository.findByCode(code).orElse(null);
            if (permission == null) {
                continue;
            }
    ...
    /** 补齐各业务页「接口权限」：挂到对应菜单下，保证权限内容可完整展示与分配。 使用 ensureMenuPermission 保证父菜单一定存在，避免菜单缺失时接口未登记。 */
    private void ensurePageApiPermissions() {
        ...
        Permission menuUnit =
                ensureMenuPermission("menu:system:unit", "单位管理", "/system/units", org, 2);
        Permission menuRole =
                ensureMenuPermission("menu:system:role", "角色列表", "/system/roles", rbac, 1);
        Permission menuPermission =
                ensureMenuPermission(
                        "menu:system:permission", "角色权限", "/system/permissions", rbac, 2);
        Permission menuPermContent =
                ensureMenuPermission(
        ...
        Permission menuRoute =
                ensureMenuPermission("menu:system:route", "路由管理", "/system/routes", rbac, 4);
        Permission menuDashboard =
                ensureMenuPermission("menu:dashboard", "首页", "/dashboard", null, 1);
        ...
        upsertApi(
                ...
                4);
        upsertApi(
                "api:GET:/api/permissions/{id}/groups",
                "菜单子权限分组",
                "GET",
                "/api/permissions/{id}/groups",
                menuPermContent,
                5);
    ...
    /** Token 续期接口授权给全部启用角色。仅空库首次种子执行。 */
    private void grantRefreshTokenToAllRoles() {
        if (!seedRolePermissions) {
            return;
        }
        Permission permission =
                permissionRepository.findByCode("api:POST:/api/auth/refresh").orElse(null);
        if (permission == null) {
            return;
        }
        for (Role role : roleRepository.findAll()) {
    ...
    private void upsertApi(
            String code, String name, String method, String path, Permission parent, int sort) {
        Permission permission = permissionRepository.findByCode(code).orElse(null);
        if (permission == null) {
    ...
    private void ensureDualCrudButtons(
            Permission menu, String prefix, boolean withAssign, boolean withAddChild) {
        // 工具栏四件套
        ...
        if (withAssign) {
            ...
            permissionRepository
                    .findByCode(prefix + ":assign")
                    .ifPresent(this::grantToPrivilegedRoles);
        }
        if (withAddChild) {
            upsertTableButton(prefix + ":add-child", "添加子级", menu, 4);
            permissionRepository
                    .findByCode(prefix + ":add-child")
                    .ifPresent(this::grantToPrivilegedRoles);
        }
    }
    ...
    private void ensureTableColumnPermissions() {
        ...
        saveApiStandalone(
                ...
                6);
        permissionRepository
                .findByCode("api:GET:/api/table-columns")
                .ifPresent(this::grantToPrivilegedRoles);
        permissionRepository
    ...
    private Map<String, Permission> initPermissions() {
        ...
        Permission menuDashboard =
                saveMenu(map, "menu:dashboard", "工作台", "/dashboard", null, ++sort);
        ...
        Permission menuRoute =
                saveMenu(map, "menu:system:route", "路由管理", "/system/routes", menuRbac, ++sort);
        Permission menuRole =
                saveMenu(map, "menu:system:role", "角色列表", "/system/roles", menuRbac, ++sort);
        Permission menuPermContent =
                saveMenu(
                        map,
                        "menu:system:permission-content",
    ...
    private void saveButton(
            Map<String, Permission> map, String code, String name, Permission parent, int sort) {
        Permission p = new Permission();
        p.setCode(code);
        p.setName(name);
                        ...
                        user -> {
                            if (user.getRoles() == null || user.getRoles().isEmpty()) {
                                ...
                                if ("SuperAdmin".equalsIgnoreCase(user.getUsername())) {
                                    ...
                                    Role adminRole =
                                            roleRepository.findByCode("ADMIN").orElse(superAdmin);
                                    roles.add(adminRole);
                                } else {
    ...
    private void ensureRoutePermissions() {
        ...
        saveApiStandalone(
                "api:GET:/api/routes/tree", "路由树接口", "GET", "/api/routes/tree", menuRoute, 1);
        saveApiStandalone(
                "api:GET:/api/routes/{id}", "路由详情接口", "GET", "/api/routes/{id}", menuRoute, 2);
        saveApiStandalone("api:POST:/api/routes", "创建路由接口", "POST", "/api/routes", menuRoute, 3);
        saveApiStandalone(
                "api:PUT:/api/routes/{id}", "更新路由接口", "PUT", "/api/routes/{id}", menuRoute, 4);
        saveApiStandalone(
                "api:DELETE:/api/routes/{id}",
                "删除路由接口",
                "DELETE",
        ...
        Permission menuDashboard = permissionRepository.findByCode("menu:dashboard").orElse(null);
        if (menuDashboard != null
                && permissionRepository.findByCode("api:GET:/api/auth/menus").isEmpty()) {
            saveApiStandalone(
                    "api:GET:/api/auth/menus",
                    "用户菜单接口",
                    "GET",
        ...
        Role superAdmin = roleRepository.findByCode("SUPER_ADMIN").orElse(null);
        Role admin = roleRepository.findByCode("ADMIN").orElse(null);
        // 仅空库首次种子时写入角色授权；已有库补齐路由权限定义后不改 sys_role_permission
        if (seedRolePermissions) {
            if (superAdmin != null) {
            ...
            if (admin != null) {
                ...
                permissionRepository
                        .findByCode("api:DELETE:/api/routes/{id}")
                        .ifPresent(perms::add);
                permissionRepository
                        .findByCode("api:POST:/api/routes/batch-delete")
                        .ifPresent(perms::add);
    ...
    private void ensurePermissionMenuRenamed() {
        permissionRepository
                .findByCode("menu:system:permission")
                .ifPresent(
                        menu -> {
    ...
    private void ensureStandardButtonPermissions() {
        ...
        Permission menuUserForImport =
                permissionRepository.findByCode("menu:system:user").orElse(null);
        if (menuUserForImport != null) {
            ...
            permissionRepository
                    .findByCode("user:sensitive:view")
                    .ifPresent(this::grantToPrivilegedRoles);
        }
    ...
    private void ensureMenuCrudButtons(
            String menuCode, String prefix, boolean withAssign, boolean withAddChild) {
    ...
    private void saveApiStandalone(
            String code, String name, String method, String path, Permission parent, int sort) {
        if (permissionRepository.findByCode(code).isPresent()) {
            return;
        }

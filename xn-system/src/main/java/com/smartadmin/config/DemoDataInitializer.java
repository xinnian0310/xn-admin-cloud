...
public class DemoDataInitializer implements CommandLineRunner {
    ...
    private static final Set<String> KEEP_UNIT_CODES = Set.of(
            "XN", "XN_XA", "XN_XA_RD", "XN_XA_MKT", "XN_XA_QA", "XN_XA_FIN", "XN_XA_OPS"
    );
    ...
    private int purgeExtraRoles() {
        Role userRole = roleRepository.findByCode("USER").orElse(null);
        List<Role> extras = roleRepository.findAll().stream()
        ...
        for (User bare : userRepository.findAll()) {
            ...
            boolean hasKeep = user.getRoles().stream().anyMatch(r -> KEEP_ROLE_CODES.contains(r.getCode()));
        ...
        for (Role extra : extras) {
            roleRepository.findByIdWithPermissions(extra.getId()).ifPresent(role -> {
    ...
    private Map<String, SysUnit> ensureOrgTree() {
        ...
        for (Node node : nodes) {
            ...
            SysUnit unit = upsertUnit(node.code(), node.name(), parent.getId(), node.sort(), false, node.desc());
            map.put(node.code(), unit);
        }
        return map;
    }

    private SysUnit upsertUnit(String code, String name, Long parentId, int sort, boolean builtIn, String desc) {
    ...
    private void bindBuiltinUsersToRoot(SysUnit root) {
        ...
        for (String username : KEEP_USERNAMES) {
            userRepository.findByUsernameWithRolesIgnoreCase(username).ifPresent(user -> {
                user.setUnit(root);
                userRepository.save(user);
            });
        }
    ...
    private void renameSeedNicknames() {
        userRepository.findByUsernameIgnoreCase("SuperAdmin").ifPresent(user -> {
    ...
    private boolean shouldKeepUser(User user) {
        ...
        return user.getRoles().stream().anyMatch(r ->
                "SUPER_ADMIN".equals(r.getCode()) || "ADMIN".equals(r.getCode()));
    }
    ...
    private int purgeObsoleteUnits(Map<String, SysUnit> keepUnits) {
        Set<Long> keepIds = keepUnits.values().stream()
                .filter(Objects::nonNull)
                .map(SysUnit::getId)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());

        List<SysUnit> obsolete = unitRepository.findAll().stream()
                .filter(u -> u.getCode() == null || !KEEP_UNIT_CODES.contains(u.getCode()))
                .sorted(Comparator
                        .comparing((SysUnit u) -> u.getParentId() == null ? 0 : 1)
                        .reversed()

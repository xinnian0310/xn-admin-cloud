...
public class RbacService {
    ...
    public List<String> getRoleCodes(User user) {
        return permissionRepository.findRoleCodesByUserId(user.getId()).stream()
                .sorted()
                .toList();
    }
    ...
    public List<String> getPermissionCodes(User user) {
        ...
        return appCacheService.getPermissionCodes(user.getId(), () ->
    ...
    public void validateRoleAssignment(List<Long> roleIds) {
        ...
        Role superAdminRole = roleRepository.findByCode(SUPER_ADMIN_CODE)
    ...
    public void validateUserEffectiveRoles(List<Long> roleIds, Long unitId) {
        ...
        if (unitId != null) {
            hasUnitRoles = unitRepository.findByIdWithRoles(unitId)
    ...
    public void syncLegacyRoleField(User user) {
        String primary = user.getRoles().stream()
                .map(Role::getCode)
    ...
    public void ensureSuperAdminExists(User user, List<Long> newRoleIds) {
        ...
        boolean wasSuperAdmin = user.getRoles().stream()

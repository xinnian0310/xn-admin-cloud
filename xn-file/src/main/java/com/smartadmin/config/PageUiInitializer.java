...
public class PageUiInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        ensureCodegenConfig();
        ensureRemoteStorageConfig();
        cleanupObsoleteSystemConfigPageUi();
    }

    /** 远程连接配置：名称 / 路径模糊搜索 */
    private void ensureRemoteStorageConfig() {
        if (pageUiConfigRepository.findByRoutePath("/system/remote-storage").isPresent()) {
            return;
        }
        SysPageUiConfig config = new SysPageUiConfig();
        config.setRoutePath("/system/remote-storage");
        config.setBuiltIn(true);
        config.setSearchConfig(
                """
                [
                  {"label":"综合查询","prop":"FuzzyWord","type":"input","placeholder":"搜索名字/路径"}
                ]
                """);
        pageUiConfigRepository.save(config);
    }

    /** 对象存储已从系统配置拆出，清理旧搜索配置 */
    private void cleanupObsoleteSystemConfigPageUi() {
        pageUiConfigRepository
                .findByRoutePath("/system/config")
                .ifPresent(pageUiConfigRepository::delete);
    }

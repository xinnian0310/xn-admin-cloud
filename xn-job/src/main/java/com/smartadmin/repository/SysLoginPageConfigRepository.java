...
public interface SysLoginPageConfigRepository extends JpaRepository<SysLoginPageConfig, Long> {
    ...
    @Query(
            "SELECT c FROM SysLoginPageConfig c WHERE (:keyword = '' OR c.name LIKE CONCAT('%', :keyword, '%'))"
                    + " AND (:status IS NULL OR c.status = :status)")
    Page<SysLoginPageConfig> search(
    ...
    @Query(
            "UPDATE SysLoginPageConfig c SET c.status = 0 WHERE c.status = 1 AND (:excludeId IS NULL OR c.id <> :excludeId)")
